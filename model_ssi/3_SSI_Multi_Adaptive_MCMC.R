...
MCMC_SSI_MULTI_ADADPT <- function(data,
  ...
  x_matrix = matrix(0, n_mcmc, dim);   
  x_matrix[1,] <- mcmc_inputs$mod_start_points; x = x_matrix[1,] #3x1 #as.matrix
  log_like_vec <- vector('numeric', n_mcmc);
  log_like_vec[1] <- LOG_LIKE_SSI_MULTI(data, x);  log_like = log_like_vec[1]
  lambda_vec <- vector('numeric', n_mcmc); lambda_vec[1] <- 1
  ...
  for(i in 2:n_mcmc) {
    ...
    #POPULATE VECTORS (ONLY STORE THINNED SAMPLE)
      if (i%%mcmc_inputs$thinning_factor == 0) {
        x_matrix[i,] = x
        log_like_vec[i] <- log_like 
        lambda_vec[i] <- lambda_i #Taking role of sigma, overall scaling constant. Sigma becomes estimate of the covariance matrix of the posterior
      }
