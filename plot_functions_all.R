...
PLOT_MCMC_GRID <- function(sim_data, mcmc_output, 
  ...
  #m1
  m1_mean = cumsum(m1_mcmc)/seq_along(m1_mcmc)
  m1_lim =  max(mcmc_plot_inputs$model_params_true$m1[[1]], max(m1_mcmc))
  #m2
  m2_mean = cumsum(m2_mcmc)/seq_along(m2_mcmc)
  m2_lim = max(mcmc_plot_inputs$model_params_true$m2[[1]], max(m2_mcmc))
  #m3
  m3_mean = cumsum(m3_mcmc)/seq_along(m3_mcmc)
  m3_lim =  max(mcmc_plot_inputs$model_params_true$m3[[1]], max(m3_mcmc))
  #r0
  r0_mean = cumsum(r0_mcmc)/seq_along(r0_mcmc)
  ...
  #i.Infections
  if(!FLAGS_LIST$DATA_AUG) inf_tite = paste0(mcmc_plot_inputs$seed_count, ', ', mcmc_plot_inputs$model_typeX, " Data, r0 = ", model_params_true$r0_start) # 'Day Infts, '
  else inf_tite = paste0(mcmc_plot_inputs$seed_count, ', ', mcmc_plot_inputs$model_typeX, " Data, r0 = ", model_params_true$r0_start, ", + Data Aug")
  ...
  plot.ts(m1_mcmc, ylab = mcmc_plot_inputs$mod_par_names[1], ylim=c(0, m1_lim),
          main = paste(mcmc_plot_inputs$mod_par_names[1], "MCMC", mcmc_plot_inputs$model_typeX,", True:", mcmc_plot_inputs$model_params_true$m1,
                       "Start: ", mcmc_plot_inputs$mod_start_points$m1),
          cex.lab=1.5, cex.axis=1.5, cex.main=1.5, cex.sub=1.5)
  abline(h = mcmc_plot_inputs$model_params_true$m1[[1]], col = 'red', lwd = 2) #True = green
  
  plot.ts(m2_mcmc, ylab = 'm2', ylim=c(0, m2_lim), 
          main = paste(mcmc_plot_inputs$mod_par_names[2], "MCMC", mcmc_plot_inputs$model_typeX,", True:", mcmc_plot_inputs$model_params_true$m2,
                       "Start: ", mcmc_plot_inputs$mod_start_points$m2),
          cex.lab=1.5, cex.axis=1.5, cex.main=1.5, cex.sub=1.5)
  abline(h = mcmc_plot_inputs$model_params_true$m2[[1]], col = 'blue', lwd = 2) #True = green
  
  plot.ts(m3_mcmc,  ylab = 'm3', ylim=c(0,m3_lim),
          main = paste(mcmc_plot_inputs$mod_par_names[3], "MCMC", mcmc_plot_inputs$model_typeX,", True:", mcmc_plot_inputs$model_params_true$m3,
                       "Start: ", mcmc_plot_inputs$mod_start_points$m3),
          cex.lab=1.5, cex.axis=1.5, cex.main=1.5, cex.sub=1.5)
  abline(h = mcmc_plot_inputs$model_params_true$m3[[1]], col = 'green', lwd = 2) #True = green
  ...
  plot(seq_along(m1_mean), m1_mean,
       ylim=c(0, m1_lim),
       xlab = 'Time', ylab =  mcmc_plot_inputs$mod_par_names[1],
       main = paste(mcmc_plot_inputs$mod_par_names[1], "MCMC mean, True:", mcmc_plot_inputs$model_params_true$m1, "Start:", mcmc_plot_inputs$mod_start_points$m1),
       cex.lab=1.5, cex.axis=1.5, cex.main=1.5, cex.sub=1.5)
  #print(plot2)
  abline(h = mcmc_plot_inputs$model_params_true$m1[[1]], col = 'red', lwd = 2)
  
  #m2 mean
  plot(seq_along(m2_mean), m2_mean,
       ylim=c(0, m2_lim),
       xlab = 'Time', ylab = 'm2',
       main = paste(mcmc_plot_inputs$mod_par_names[2], "MCMC mean, True:", mcmc_plot_inputs$model_params_true$m2, "Start:", mcmc_plot_inputs$mod_start_points$m2),
       cex.lab=1.5, cex.axis=1.5, cex.main=1.5, cex.sub=1.5)
  #print(plot2)
  abline(h = mcmc_plot_inputs$model_params_true$m2[[1]], col = 'blue', lwd = 2)
  
  #m3 Mean
  plot(seq_along(m3_mean), m3_mean,
       xlab = 'Time', ylab = 'm3', 
       main = paste(mcmc_plot_inputs$mod_par_names[3], "MCMC mean, True:",mcmc_plot_inputs$model_params_true$m3, "Start:", mcmc_plot_inputs$mod_start_points$m3),
       ylim=c(0, m3_lim),
       cex.lab=1.5, cex.axis=1.5, cex.main=1.5, cex.sub=1.5)
  abline(h = mcmc_plot_inputs$model_params_true$m3[[1]], col = 'green', lwd = 2)
  ...
  hist(m1_mcmc, freq = FALSE, breaks = 100,
       ...
       main = paste(mcmc_plot_inputs$mod_par_names[1],
                    "True:", mcmc_plot_inputs$model_params_true$m1, " prior:", m1_prior),
       xlim=c(0, m1_lim),
       cex.lab=1.5, cex.axis=1.5, cex.main=1.5, cex.sub=1.5)
  abline(v = mcmc_plot_inputs$model_params_true$m1[[1]], col = 'red', lwd = 2)
  ...
  hist(m2_mcmc, freq = FALSE, breaks = 100,
       ...
       main = paste(mcmc_plot_inputs$mod_par_names[2],
                    "True:", mcmc_plot_inputs$model_params_true$m2, " prior:", m2_prior),
       xlim=c(0, m2_lim),
       cex.lab=1.5, cex.axis=1.5, cex.main=1.5, cex.sub=1.5)
  abline(v = mcmc_plot_inputs$model_params_true$m2[[1]], col = 'blue', lwd = 2)
  ...
  hist(m3_mcmc, freq = FALSE, breaks = 100,
       ...
       main = paste(mcmc_plot_inputs$mod_par_names[3],
                    "True:", mcmc_plot_inputs$model_params_true$m3, " prior:", m3_prior),
       xlim=c(0, m3_lim),
       cex.lab=1.5, cex.axis=1.5, cex.main=1.5, cex.sub=1.5)
  abline(v = mcmc_plot_inputs$model_params_true$m3[[1]], col = 'green', lwd = 2)#
  ...
  hist(r0_mcmc, freq = FALSE, breaks = 100,
       ...
       main = paste('R0 total, True:', r0_start), #'prior: ***'), xlim=c(0, r0_lim),
       cex.lab=1.5, cex.axis=1.5, cex.main=1.5, cex.sub=1.5)
  ...
  if (FLAGS_LIST$RJMCMC){
    ...
    hist(m1_mcmc, freq = FALSE, breaks = 100,
         xlab = mcmc_plot_inputs$mod_par_names[1], #ylab = 'Density', 
         main = paste("m1, True m1 = ", mcmc_plot_inputs$model_params_true$m1[[1]]), 
         xlim=c(0, m1_lim),
         cex.lab=1.5, cex.axis=1.5, cex.main=1.5, cex.sub=1.5)
    abline(v = mcmc_plot_inputs$model_params_true$m1[[1]], col = 'red', lwd = 2)
    ...
    df_results <- data.frame(
      ...
      n_mcmc = n_mcmc,
      m1 = mcmc_plot_inputs$model_params_true$m1[[1]],
      a_mc = a_mcmc_mean,
      m2 = mcmc_plot_inputs$model_params_true$m2[[1]],
      b_mc = b_mcmc_mean,
      m3 = mcmc_plot_inputs$model_params_true$m3[[1]],
      c_mc = c_mcmc_mean,
      R0 = r0_start, 
      R0_mc = r0_mcmc_mean,
...
PLOT_MCMC_GRID_REAL_DATA <- function(sim_data, mcmc_output, 
  ...
  m2_mean = cumsum(m2_mcmc)/seq_along(m2_mcmc)
  m2_lim = max(mcmc_plot_inputs$mod_start_points$m2[[1]], max(m2_mcmc))
  m2_lim2 = max(mcmc_plot_inputs$mod_start_points$m2[[1]], m2_mean)
  
  #m3
  m3_mean = cumsum(m3_mcmc)/seq_along(m3_mcmc)
  m3_lim =  max(mcmc_plot_inputs$mod_start_points$m3[[1]], max(m3_mcmc))
  m3_lim2 =  max(mcmc_plot_inputs$mod_start_points$m3[[1]], m3_mean) 
  ...
  df_results <- data.frame(
    ...
    m3 = mcmc_plot_inputs$mod_start_points$m3[[1]],
    m3_mc = m3_mean_tail,
    R0 = mcmc_plot_inputs$mod_start_points$r0_start, 
    R0_mc = m4_mean_tail,
    accept_rate_m1 = round(mcmc_output$list_accept_rates$accept_rate1, 2),
    a_rte_m2 = round(mcmc_output$list_accept_rates$accept_rate2, 2),
    a_rte_m3 = round(mcmc_output$list_accept_rates$accept_rate3, 2),
