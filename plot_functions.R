...
PLOT_MCMC_GRID <- function(sim_data, mcmc_output, 
  ...
  #m1
  m1_mean = cumsum(m1_mcmc)/seq_along(m1_mcmc)
  a_lim =  max(mcmc_plot_inputs$model_params_true$m1[[1]], max(m1_mcmc))
  #m2
  m2_mean = cumsum(m2_mcmc)/seq_along(m2_mcmc)
  b_lim = max(mcmc_plot_inputs$model_params_true$m2[[1]], max(m2_mcmc))
  #m3
  m3_mean = cumsum(m3_mcmc)/seq_along(m3_mcmc)
...
PLOT_MCMC_GRID_REAL_DATA <- function(sim_data, mcmc_output, 
  ...
  m2_mean = cumsum(m2_mcmc)/seq_along(m2_mcmc)
  b_lim = max(mcmc_plot_inputs$mod_start_points$m2[[1]], max(m2_mcmc))
  b_lim2 = max(mcmc_plot_inputs$mod_start_points$m2[[1]], m2_mean)
